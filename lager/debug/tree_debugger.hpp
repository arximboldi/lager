    ...
    struct model
    {
            ...
            auto new_branches = branches.update(pos.branch, [&] (auto history) {
                if (next_index < cursor.size()) {
                    ...
                    if (pos.step + 1 == history.size()) {
                        new_cursor = cursor.set(
                            cursor_index, {pos.branch, pos.step + 1});
                        return history.push_back(step{act, m, {}});
                    } else {
                        new_cursor = cursor.push_back({0, 0});
                        return history.update(pos.step, [&] (auto node_) {
                            auto node = *node_;
                            node.branches = node.branches.push_back(
                                {step{act, m, {}}});
                            return node;
                        });
                    }
        ...
        void append(const Action& act, const Model& m)
        {
            ...
            if (cursor.empty()) {
                ...
                tie(branches, cursor) = do_append(branches, cursor, 0,
                                                  act, m);
            }
        }
        ...
        return std::visit(visitor{
                ...
                [&] (undo_action) -> result_t {
                    if (!m.cursor.empty()) {
                        ...
                        m.cursor = pos.step > 0
                            ? m.cursor.set(index, {pos.branch, pos.step - 1})
                            : m.cursor.take(index);
                    }
                    return {m, noop};
                },
